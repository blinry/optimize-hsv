...
int main(int argc, char const *argv[]) {
    ...
    string target_img = source_img+"-optimized.png";
    ...
    while (!o.done()) {
        ...
        v2 << v[2];
        string cmd;
        cmd = (convert_cmd+source_img).append(" -modulate ").append(v0.str()).append(",").append(v1.str()).append(",").append(v2.str()).append(" ").append(target_img);
        cout << cmd << endl;
        system(cmd.c_str());
